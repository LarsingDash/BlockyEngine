...
The Blocky Engine is a modular C++ game engine which can be used for 2D game design. Blocky Engine can be used without
...
libraries (.dll/.so), found by some
sort of config file. Custom implementations of these modules could be made by following the same module interface, and
...
reset these options (which is not necessary)

## Installation

To run the Blocky Engine, download this project from GitHub and have CLion installed (or any other IDE that supports
CMake). Open the project using a MinGW toolchain configuration. When cloning the engine it should, on its own, be able
...
After correctly installing the required libraries (as listed below) everything will be handled in the root CMake file.
...
`lib` folders from `x86_64-w64-mingw32` into `./libs/SDL2/`. The `PATH` environment variable needs to be pointing to
this bin folder. This can be
done in CLion by setting the variable in the run configuration to `..\libs\SDL2\bin;` (make sure to enable include
...
should be placed in `./libs/SDL2_gfx/` in addition to the following CMakeLists.txt:
...
contents of this zip file should be extracted to `./libs/glm/`.
